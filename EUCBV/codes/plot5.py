...
class plot4(object):
    ...
    out_file='NewExpt/expt5/plotFinal'
    ...
    def fileReadWrite(self,id,name,filename,actionRegret):
        ...
        f = open(str(self.out_file)+str(name)+".txt", 'w')
    ...
    #def regretWeightsGraph(filename,filename1,filename2):
        with open(str(self.out_file)+str(name)+".txt", 'r') as infile:
    ...
    def regretWeightsGraph(self,filename):
        ...
        self.limit=2
        #self.timestep=60000
        self.timestep=400000
        #self.timestep=3649236
        ...
        process=multiprocessing.Process(target=self.fileReadWrite,args=(0,takeName,filename, self.actionRegret))
        jobs.append(process)
        '''
        ...
        plt.ylabel('Cumulative Regret',fontsize=18)
        plt.xlabel('timesteps',fontsize=18)
        ax1.plot(scale, self.actionRegret,'g+',label="ClusUCB(No Clustering,p=1)")
        '''
...
if __name__ == "__main__":
    ...
    obj.regretWeightsGraph('NewExpt/expt5/testRegretBU01.txt')
